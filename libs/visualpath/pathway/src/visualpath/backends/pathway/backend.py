...
class PathwayBackend(ExecutionBackend):
    ...
    def run(
        ...
        Returns:
            List of Trigger objects that fired.
        """
        from visualpath.flow.graph import FlowGraph
        ...
        # Create FlowGraph from modules
        graph = FlowGraph.from_modules(modules)

        # Register trigger callback on graph if provided
        if on_trigger is not None:
            graph.on_trigger(on_trigger)

        # Execute and return triggers
