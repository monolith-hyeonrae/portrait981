...
class _BaseProcess:
    ...
    def _emit_trace(self, record_type: str, frame_id: int, data: Dict[str, Any]) -> None:
...
class ExtractorProcess(_BaseProcess):
    ...
    def _emit_frame_drop(self, dropped_frame_ids: List[int]) -> None:
        ...
        self._emit_trace("frame_drop", dropped_frame_ids[0] if dropped_frame_ids else 0, {
            "dropped_frame_ids": dropped_frame_ids, "reason": "gap_in_sequence",
        })
...
class FusionProcess(_BaseProcess):
    ...
    def _emit_sync_delay(
        ...
        self._emit_trace("sync_delay", frame_id, {
        ...
        for src, msg in obs_dict.items():
            ...
            if observation:
                try:
                    ...
                    result = self._fusion.process(mock_frame, deps)
                    if result is not None and result.should_trigger and result.trigger:
