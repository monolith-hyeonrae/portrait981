...
class TestPathNodeSpec:
    ...
    def test_returns_module_spec(self):
        ...
        node = PathNode(name="analysis", modules=[ext])
        spec = node.spec
        assert isinstance(spec, ModuleSpec)

    def test_spec_modules(self):
        ext1 = DummyModule("face")
        ext2 = DummyModule("pose")
        node = PathNode(name="analysis", modules=[ext1, ext2])
        spec = node.spec
        assert len(spec.modules) == 2
        assert spec.modules[0] is ext1
        assert spec.modules[1] is ext2

    def test_spec_with_trigger(self):
        ext = DummyModule("face")
        trigger = DummyTriggerModule()
        node = PathNode(name="analysis", modules=[ext, trigger])
        spec = node.spec
        assert len(spec.modules) == 2
    ...
    def test_spec_join_window_ns_custom(self):
        ext = DummyModule("face")
        node = PathNode(name="analysis", modules=[ext], join_window_ns=200_000_000)
    ...
    def test_spec_is_frozen(self):
        ...
        node = PathNode(name="analysis", modules=[ext])
        with pytest.raises(FrozenInstanceError):
            node.spec.parallel = True

    def test_spec_modules_are_tuple(self):
        ext = DummyModule("face")
        node = PathNode(name="analysis", modules=[ext])
        assert isinstance(node.spec.modules, tuple)
...
class TestSpecInheritance:
    ...
    def test_all_specs_inherit_from_node_spec(self):
        spec_types = [
            SourceSpec, ModuleSpec, FilterSpec, ObservationFilterSpec,
...
class TestSpecExports:
    ...
    def test_specs_importable_from_flow(self):
        from visualpath.flow import (
            NodeSpec, SourceSpec, ModuleSpec, FilterSpec,
...
class TestSpecProcessConsistency:
    ...
    def test_path_node_spec_modules_immutable(self):
        """PathNode spec.modules is a tuple (immutable)."""
        ext = DummyModule("face")
        node = PathNode(name="p", modules=[ext])
        spec = node.spec
