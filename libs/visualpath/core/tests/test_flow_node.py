...
class TestPathNode:
    ...
    def test_with_modules(self):
        ...
        node = PathNode(name="test_path", modules=[ext])
        assert node.name == "test_path"
        assert node.modules == (ext,)

    def test_with_modules_and_trigger(self):
        ext = CountingModule("ext1")
        trigger = ThresholdTriggerModule()
        node = PathNode(name="my_path", modules=[ext, trigger])
        assert node.name == "my_path"

    def test_interpret_adds_observations(self):
        ext = CountingModule("ext1", return_value=0.7)
        node = PathNode(name="test", modules=[ext])
        interp = SimpleInterpreter()
    ...
    def test_interpret_runs_trigger(self):
        ext = CountingModule("ext1", return_value=0.7)
        trigger = ThresholdTriggerModule(threshold=0.5, depends_on="ext1")
        node = PathNode(name="test", modules=[ext, trigger])
        interp = SimpleInterpreter()
    ...
    def test_updates_path_id(self):
        ext = CountingModule("ext1")
        node = PathNode(name="human", modules=[ext])
        interp = SimpleInterpreter()
    ...
    def test_requires_name_or_modules(self):
        with pytest.raises(ValueError):
            PathNode()
