...
class TestFlowGraph:
    ...
    def test_context_manager(self):
        ...
        path_node = PathNode(name="test", modules=[ext])
...
class TestGraphExecutor:
    ...
    def test_trigger_callback(self):
        ...
        path_node = PathNode(name="test", modules=[ext, trigger])
...
class TestFlowGraphBuilder:
    ...
    def test_build_with_modules(self):
        """Test building with modules."""
        ext = CountingModule("ext1")

        graph = (FlowGraphBuilder()
            .register_module("ext1", ext)
            .source("frames")
            .path("test", modules=["ext1"])
            .build())
    ...
    def test_build_with_join(self):
        ...
        graph = (FlowGraphBuilder()
            .source("frames")
            .fanout(paths=["a", "b"])
            .path("a", modules=[ext_a])
            .path("b", modules=[ext_b])
            .join(["a", "b"])
            .build())
...
class TestFlowIntegration:
    ...
    def test_branching_pipeline(self):
        ...
        graph = (FlowGraphBuilder()
            ...
            .path("human", modules=[ext_human])
            .path("scene", modules=[ext_scene])
            .build())
        ...
        assert ext_human._process_count == 2
        assert ext_scene._process_count == 2

    def test_from_modules_basic(self):
        """Test FlowGraph.from_modules() creates correct graph."""
        ext = CountingModule("ext1", return_value=0.7)
    ...
    def test_from_modules_with_trigger(self):
        """Test FlowGraph.from_modules() with trigger module."""
        ext = CountingModule("ext1", return_value=0.7)
        trigger = ThresholdTriggerModule(threshold=0.5, depends_on="ext1")

        graph = FlowGraph.from_modules([ext, trigger])

        assert "source" in graph.nodes
        assert "pipeline" in graph.nodes

    def test_from_modules_with_on_trigger(self):
        """Test FlowGraph.from_modules() with on_trigger callback."""
        ext = CountingModule("ext1", return_value=0.7)
        triggered = []
    ...
    def test_from_modules_multiple_modules(self):
        """Test FlowGraph.from_modules() with multiple modules."""
    ...
    def test_parallel_paths_with_join(self):
        ...
        assert ext_a._process_count == 1
        assert ext_b._process_count == 1
