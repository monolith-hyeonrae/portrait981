...
def _get_registered(
    ...
    discover_fn_name: str,
) -> Optional[Module]:
    """Look up a module by name in registry, falling back to plugin discovery."""
    if name in registry:
        return registry[name]
    try:
        from visualpath.plugin import discovery
        discovered = getattr(discovery, discover_fn_name)()
        if name in discovered:
            # discovered values are EntryPoint objects — load() to get class
            cls = discovered[name].load()
            return cls()
...
def _list_registered(
    ...
    """List all available names from registry + plugin discovery."""
    names = list(registry.keys())
    try:
        from visualpath.plugin import discovery
        names.extend(getattr(discovery, discover_fn_name)().keys())
    except (ImportError, AttributeError):
        pass
...
class FunctionFusion(Module):
    ...
    def process(
        self, frame: Frame, deps: Optional[Dict[str, Observation]] = None
    ) -> Observation:
