...
from momentscan.face_quality.output import FaceQualityOutput, FaceQualityResult
    ...
    def process(
        ...
        return Observation(
            ...
            signals={
                ...
                "face_blur": float(output.face_blur),
                "face_exposure": float(output.face_exposure),
                "face_contrast": float(output.face_contrast),
                "clipped_ratio": float(output.clipped_ratio),
                "crushed_ratio": float(output.crushed_ratio),
                "parsing_coverage": float(output.parsing_coverage),
                "seg_face": float(output.seg_face),
                "seg_eye": float(output.seg_eye),
                "seg_mouth": float(output.seg_mouth),
                "seg_hair": float(output.seg_hair),
            },
            data=output,
            metadata={
                "_metrics": {
