...
from momentscan.face_baseline.output import (
    FaceBaselineProfile,
    FaceBaselineOutput,
)
...
class FaceBaselineAnalyzer(Module):
    ...
    def process(
        ...
        return Observation(
            ...
            signals={
                "profiles_count": float(len(profiles)),
                "main_converged": 1.0 if main_profile is not None else 0.0,
                "passenger_converged": 1.0 if passenger_profile is not None else 0.0,
            },
            data=output,
        )
