...
class PortraitScoreAnalyzer(Module):
    ...
    def initialize(self) -> None:
        ...
        if self._enable_aesthetic:
            try:
                from momentscan.portrait_score.backends.clip_portrait import CLIPPortraitScorer
    ...
    def process(
        ...
        if clip_axes:
            for ax in clip_axes:
                if ax.active:
                    axis_signals[f"clip_axis_{ax.name}"] = 1.0
            axis_signals["clip_quirky"] = float(any(
                ax.active and ax.action == "quirky" for ax in clip_axes
            ))
            axis_signals["clip_select"] = float(any(
        ...
        return Observation(
            ...
            t_ns=frame.t_src_ns,
            signals={
                "has_aesthetic": float(output.head_aesthetic > 0),
                **axis_signals,
            },
            data=output,
            metadata={
