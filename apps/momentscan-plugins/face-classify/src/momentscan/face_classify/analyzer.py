...
class FaceClassifierAnalyzer(Module):
    ...
    def annotate(self, obs):
        """Return BBoxMark with role-based colors for each classified face."""
        if obs is None or obs.data is None:
            return []
        from vpx.sdk.marks import BBoxMark, LabelMark

        if not hasattr(obs.data, "faces") or not obs.data.faces:
            return []

        marks = []
        for cf in obs.data.faces:
            face, role = cf.face, cf.role
    ...
    def process(
        ...
        if not faces:
            return Observation(
                ...
                metadata={"_metrics": {"faces_classified": 0, "tracks_active": 0}},
            )
        ...
        return Observation(
            ...
            t_ns=frame.t_src_ns,
            signals={
                "main_detected": 1.0 if result["main_face"] else 0.0,
                "passenger_count": float(len(result["passenger_faces"])),
                "transient_count": float(result["transient_count"]),
                "noise_count": float(result["noise_count"]),
                "total_faces": float(len(result["faces"])),
            },
            data=FaceClassifierOutput(
                faces=result["faces"],
            ...
            metadata={
                "_metrics": {
                    "faces_classified": len(result["faces"]),
                    "tracks_active": len(self._track_history),
                },
            },
            timing=timing,
        )
