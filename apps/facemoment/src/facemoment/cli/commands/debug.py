...
Uses the unified ``fm.run(on_frame=handler)`` execution path so that
...
def run_debug(args):
    ...
    of fm.run() so we can initialize workers before printing the header,
    ...
    from facemoment.cli.utils import detect_distributed_mode
    ...
    # Resolve modules via FacemomentApp
    app = FacemomentApp(cooldown=2.0)
    ...
    from facemoment.main import Result
...
def _resolve_selected_to_names(selected: List[str], args) -> List[str]:
    """Convert parsed analyzer selection to fm.run() analyzer names.
    ...
    Returns:
        List of analyzer names for fm.run().
    """
    if 'all' in selected:
