...
Uses the unified ``fm.run()`` execution path for both normal and
distributed modes. Distributed mode builds an IsolationConfig and
passes it to ``fm.run(isolation=...)``.
"""
...
def run_process(args):
    ...
    All modes use the unified execution path:
        fm.run() -> vp.run() -> Backend.execute()

    Distributed mode (--distributed, --venv-*) builds IsolationConfig
    and passes it to fm.run(isolation=...), which auto-switches to
    the worker backend.
    """
    ...
    analyzer_names = None  # fm.run() default
