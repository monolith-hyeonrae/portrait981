...
> momentscan, personmemory, reportrait이 공유하는 인터페이스, 타입, 기준값 정의.
...
1. **momentscan → personmemory**: identity set 이미지 + 임베딩 전달, match/update API 호출
2. **personmemory → reportrait**: select_refs() → reference image 경로 전달
...
| 5 | personmemory | Phase 3 출력을 저장/관리하는 핵심 인프라 |
...
```
momentscan Phase 1 → Phase 2 shadow → personmemory → reportrait
```
