...
## 핵심 통찰: Threshold는 추상 개념의 축별 독립 근사이다
...
이 추상 개념의 경계를 각 feature space에서 **축별 독립 근사(marginal approximation)**한 것이다.

```
올바른 방향:
  추상 개념 ("warm_smile") → 개념 공간에서 경계 정의 → 각 feature space로 축별 독립 근사
...
**"warm_smile" 개념의 경계를 expression 공간의 happy 축에서 독립 근사한 값**이다.
...
**추상 개념에 의한 selection이 이들의 출력 사이에 상관을 유도한다 (Berkson's paradox).**
이는 통계학에서 잘 알려진 현상으로, 선택 사건(concept membership)에 조건을 걸면
주변적으로 독립인 변수들 사이에 상관이 나타난다.
...
**하지만 "밝은 미소"라는 개념으로 selection하면 얼굴 근육, 감정, 포즈 사이에 상관이 유도된다.**

추상 개념의 경계를 각 축에서 독립적으로 근사하면, 이 상관이 소실된다:
...
근본적 원인:  개념 selection이 모듈 출력 간 상관을 유도 (Berkson's paradox)
              → selection에 의해 유도되는 의존성
...
= 추상 개념의 결정 경계 (자동 도출)
     ↓
축별 근사: 학습된 경계를 각 축으로 투영하면 threshold가 자동 도출됨
           (해석/디버깅/운용자 조정용)
```

**Threshold는 입력이 아니라, 학습된 결정 경계에서 도출된 부산물이다.**
...
| **Backward** | 학습된 경계 → 각 축으로 투영 → threshold 도출 | 해석 가능한 설명 |
| **Adjustment** | 운용자가 도출된 threshold를 조정 → 재학습 | 개념 미세 조정 |
...
| 상관 원인 | LF 설계 중복 | **개념 조건부 유도 상관 (Berkson's paradox)** |
...
추상 개념의 경계를 각 출력 공간에서 축별로 근사하면서 비로소 같은 질문이 된다.**

Crowds는 annotator가 이미 같은 공간에 있다고 가정한다.
또한 annotator 간 독립성을 가정하지만, VisualBind에서는
**개념 selection이 observer 간 상관을 유도한다 (Berkson's paradox)** — 이것은 모델을 다양화해도 제거할 수 없다.
...
| 상관 원인 | annotator 간 독립 가정 | **개념 조건부 유도 상관 (Berkson's paradox)** |
| Downstream | 단일 task | **multi-bucket + few-shot transfer** |
| 핵심 차이 | annotator가 이미 같은 공간 | **이종 공간의 축별 독립 근사 + 근사 시 소실된 cross-modal 상관 복원** |

Rodrigues & Pereira가 다루지 않는 것:
축별 독립 근사 과정에서 발생하는 정보 손실, 축 간 독립 가정에 의한 cross-modal 상관 소실,
...
### Student가 배우는 것 = 개념 selection이 유도한 상관 구조
...
Student가 학습하는 것:
  ...
  (3) 개념 selection에 의해 유도된 teacher 간 cross-modal 상관
...
개별 threshold가 독립적으로 근사하면서 잃어버린 cross-modal 상관을,
Student가 joint space에서 복원하는 것이다.

Student가 teacher를 넘을 수 있는 세 가지 메커니즘:

1. **Cross-modal 상관 복원** — 개념 selection이 유도한 축 간 관계를 joint space에서 학습. 독립 threshold는 구조적으로 포착 불가.
...
추상 개념의 축별 독립 근사 과정을 인식하면, 시스템이 자연스럽게 세 층으로 분리된다.
...
catalog 메커니즘과 personmemory로 다양성을 저장하고,
**VisualBind의 multi-bucket + few-shot이 기준 변경에 빠르게 적응하는 메커니즘**이 된다.

```
personmemory  — 다양성 저장 (모든 가능성을 보존)
...
> VisualBind의 observer는 이종 공간에 있으며, threshold **축별 독립 근사**를 통해 같은 공간으로 온다.
> 이 근사 과정과 정보 손실을 다루는 것이 기존 crowds에 없는 부분이다.
...
1. **Projected Crowds 프레임워크**: 추상 개념의 경계를 각 이종 feature space에서 축별 독립 근사할 때 발생하는 정보 손실(marginal 근사에 의한 cross-modal 상관 소실)을 형식화하고, 근사 전 연속 정보와 근사 후 이산 투표를 dual-mode로 동시 활용하여 이를 복구하는 프레임워크. Multi-modal fusion과 learning from crowds를 통합한다.
...
3. **추상 개념의 축별 근사 = 개념 정의**: Concept drift를 사영 공간의 변화로 재해석. 운용자가 threshold를 변경하여 의도적으로 개념을 재정의할 수 있고, Student가 자동으로 새 개념에 적응하는 메커니즘.
...
개별 메커니즘은 기존에 알려져 있다. Novelty는 **추상 개념의 축별 독립 근사 과정을 형식화하고, 그 한계를 극복하는 프레임워크**에 있다:
- 추상 개념 경계의 축별 독립 근사(threshold)가 만드는 정보 손실과 cross-modal 상관 소실을 형식화
- 근사 전/후 정보를 dual-mode loss로 동시 활용하여 상관 복원
- 근사 변경(threshold 조정)으로 concept drift에 대응하는 3층 아키텍처
...
visualbind  = 지금 이 순간의 판단 (정적)
              signals (43D), VisualBind judge (HeuristicStrategy + TreeStrategy)
              "관찰 결과를 어떻게 판단할지"
