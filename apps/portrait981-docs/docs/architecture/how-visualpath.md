...
3-app 구조: `momentscan (분석/수집) → personmemory (저장/관리) → reportrait (AI 생성)`
...
momentscan의 Phase별 + personmemory가 공통으로 필요한 것들이 있습니다. 중복 구현하면 계약이 어긋나므로, `vpx-sdk` 또는 공용 위치에 한 번 구현해야 합니다:
...
personmemory는 앱이라기보다 **라이브러리**(memory bank API)에 가깝습니다. momentscan이 `update()`를 호출하고, reportrait이 `select_refs()`를 호출합니다.

선택지:
- `apps/personmemory/` — 독립 앱 + CLI
- `libs/personmemory/` — 라이브러리, 다른 앱이 import
