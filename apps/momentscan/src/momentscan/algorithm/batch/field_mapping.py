...
PIPELINE_FIELD_MAPPINGS: tuple[FieldMapping, ...] = (
    ...
    # face.quality (2)
    FieldMapping("face.quality", "face_blur", "얼굴 선명도 (Laplacian)", "gate+quality", "quality_face_blur_weight",
                 rationale="얼굴 마스크 내 선명도. gate: 30 미만 탈락, quality: 가중 품질 점수"),
    FieldMapping("face.quality", "face_exposure", "얼굴 밝기 (0-255)", "info",
                 rationale="얼굴 crop 내 노출 참고용"),
    FieldMapping("face.quality", "face_contrast", "얼굴 로컬 대비 (CV)", "gate",
    ...
    FieldMapping("face.quality", "parsing_coverage", "얼굴 분할 커버리지", "gate",
                 rationale="BiSeNet 파싱 마스크 면적 / crop 면적. 과/저노출 시 급감하여 노출 불량 proxy"),
    # face.quality — semantic segmentation ratios (4)
    FieldMapping("face.quality", "seg_face", "얼굴 영역 비율", "info",
                 rationale="skin+brow+eye+glasses+ear+nose+mouth+lip+neck (classes 1-8,10-14). 과노출 시 seg 경계 무너짐 → 비율 변화"),
    FieldMapping("face.quality", "seg_eye", "눈 비율", "info",
                 rationale="class_map classes 4+5. 선글라스/눈감김 시 감소"),
    FieldMapping("face.quality", "seg_mouth", "입 비율", "info",
                 rationale="class_map classes 11+12+13. 마스크 착용 시 거의 0 (2명 케이스 확인). 마스크 착용 proxy 후보"),
    FieldMapping("face.quality", "seg_hair", "머리카락 비율", "info",
                 rationale="class_map class 17. face bbox 내 머리카락 비율"),
    FieldMapping("face.quality", "eye_pixel_ratio", "눈 픽셀 비율 (eye_open 교차검증)", "info",
                 rationale="= seg_eye. eye_open_ratio와 교차검증용"),
    # portrait.score (1)
    ...
    FieldMapping("frame.quality", "blur_score", "선명도 (Laplacian 분산)", "gate",
                 rationale="모션블러가 심한 프레임은 사진 품질 열화. Laplacian 50 미만 탈락 (gate_blur_min). 품질 점수는 face.quality face_blur 사용"),
    ...
    # face.gate — passenger (7)
    FieldMapping("face.gate", "passenger_detected", "동승자 검출 여부", "info",
                 rationale="face.gate analyzer에서 passenger role 할당된 얼굴 존재 여부"),
    FieldMapping("face.gate", "passenger_suitability", "동승자 적합성 점수 (0–1)", "bonus",
                 rationale="confidence × parsing soft threshold. 최종 점수에 additive bonus"),
    FieldMapping("face.gate", "passenger_confidence", "동승자 검출 신뢰도 (raw)", "info",
                 rationale="동승자 face.detect confidence (디버깅용)"),
    FieldMapping("face.gate", "passenger_parsing_coverage", "동승자 파싱 커버리지 (raw)", "info",
                 rationale="동승자 BiSeNet parsing coverage (디버깅용)"),
    FieldMapping("face.gate", "passenger_face_area_ratio", "동승자 얼굴 영역 비율", "info",
                 rationale="동승자 얼굴 크기"),
    FieldMapping("face.gate", "passenger_face_blur", "동승자 얼굴 선명도", "info",
                 rationale="동승자 얼굴 crop 선명도"),
    FieldMapping("face.gate", "passenger_face_exposure", "동승자 얼굴 밝기", "info",
