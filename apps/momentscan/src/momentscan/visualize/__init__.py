...
from momentscan.visualize.timeline_panel import TimelinePanel
from momentscan.visualize.components import (  # noqa: F401
    COLOR_DARK_BGR,
    COLOR_WHITE_BGR,
    COLOR_GRAY_BGR,
    COLOR_RED_BGR,
...
class AnalyzerVisualizer:
    """Legacy wrapper around annotate()+render_marks() for backward compatibility.
    ...
    def __init__(self, config: Optional[VisualizationConfig] = None):
        ...
        # Lightweight module instances for annotate() — no backends needed
        self._modules: Dict[str, object] = {}
        try:
            from vpx.face_detect.analyzer import FaceDetectionAnalyzer
            self._modules["face.detect"] = FaceDetectionAnalyzer(face_backend=None)
        except ImportError:
            pass
        try:
    ...
    def draw_face_observation(self, image: np.ndarray, observation: Observation) -> np.ndarray:
        ...
        output = image.copy()
        module = self._modules.get("face.detect")
        if module:
            marks = module.annotate(observation)
            output = render_marks(output, marks)
        # Summary text for backward compat
        h, w = output.shape[:2]
    ...
    def draw_face_classifier_observation(
        ...
        output = image.copy()
        module = self._modules.get("face.classify")
        if module:
            marks = module.annotate(classifier_obs)
            output = render_marks(output, marks)
    ...
    def draw_pose_observation(self, image: np.ndarray, observation: Observation) -> np.ndarray:
        ...
        output = image.copy()
        module = self._modules.get("body.pose")
        if module:
            marks = module.annotate(observation)
            output = render_marks(output, marks)
        # Summary text
...
class DebugVisualizer:
    ...
    def __init__(
        ...
        self._video_panel = VideoPanel(modules=modules)
    ...
    def render(self, ctx: RenderContext) -> np.ndarray:
        ...
        return self.create_debug_view(
            ...
            face_quality_obs=ctx.observations.get("face.quality"),
        )
    ...
    def create_debug_view(
        ...
        face_quality_obs: Optional[Observation] = None,
    ) -> np.ndarray:
        ...
        # 2. Build observations dict for VideoPanel
        ...
        if face_quality_obs is not None:
            observations["face.quality"] = face_quality_obs
        ...
        # 4. Stats panel (skip drawing if STATS layer is off)
