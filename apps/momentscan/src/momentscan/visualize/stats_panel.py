...
class StatsPanel:
    ...
    def _draw_monitor_stats(
        ...
        analyzer_metrics = stats.get("analyzer_metrics", {})
        ...
        for name, ms in ext_timings.items():
            ...
            # Show module-internal metrics summary if available
            m = analyzer_metrics.get(name)
            if m:
                summary = self._format_metrics_summary(name, m)
                if summary:
                    cv2.putText(canvas, f"  {summary}", (x, y), FONT, 0.28, COLOR_GRAY_BGR, 1)
                    y += line_h - 4
    ...
    @staticmethod
    def _format_metrics_summary(name: str, metrics: Dict) -> str:
        """Format a one-line summary of module-internal metrics."""
        parts = []
        # face.detect
        if "detection_count" in metrics:
            parts.append(f"{metrics['detection_count']} det")
        if "avg_confidence" in metrics:
            parts.append(f"c={metrics['avg_confidence']:.2f}")
        if "tracking_active" in metrics:
            parts.append(f"trk={metrics['tracking_active']}")
        # face.expression
        if "faces_analyzed" in metrics:
            parts.append(f"{metrics['faces_analyzed']} analyzed")
        # face.classify
        if "faces_classified" in metrics:
            parts.append(f"{metrics['faces_classified']} cls")
        if "tracks_active" in metrics:
            parts.append(f"trk={metrics['tracks_active']}")
        # body.pose
        if "poses_detected" in metrics:
            parts.append(f"{metrics['poses_detected']} poses")
        if "avg_keypoint_conf" in metrics:
            parts.append(f"kc={metrics['avg_keypoint_conf']:.2f}")
        # hand.gesture
        if "hands_detected" in metrics:
            parts.append(f"{metrics['hands_detected']} hands")
        if "gestures_recognized" in metrics:
            parts.append(f"{metrics['gestures_recognized']} gest")
        # frame.quality
        if "quality_score" in metrics:
            parts.append(f"q={metrics['quality_score']:.2f}")
        if "gate_open" in metrics:
            parts.append("gate:Y" if metrics["gate_open"] else "gate:N")
        return ", ".join(parts)
