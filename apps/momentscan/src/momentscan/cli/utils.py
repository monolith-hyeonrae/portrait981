...
4. Python 로거 레벨 조정 (configure_log_levels)                  ← 이 파일
   ...
   - momentscan.main                      : CUDA conflict 등 내부 로그
   - momentscan.algorithm.analyzers: classifier init/cleanup
   - visualpath.backends.pathway           : pathway 내부 로그
...
def configure_log_levels():
    ...
    for name in (
        "vpx",
        "momentscan.main",
        "momentscan.algorithm.analyzers",
...
def setup_observability(trace_level: str, trace_output: str = None):
    ...
    from momentscan.cli.sinks import ConsoleSink
...
def detect_distributed_mode(args) -> bool:
    ...
    Returns True if --distributed or --venv-* is specified.
    """
    ...
    if venv_face or venv_pose or venv_gesture:
        distributed = True
...
def probe_analyzers(device="cuda:0", roi=None) -> dict:
    ...
    Returns:
        ...
        and "face_mode" ("enabled" or "disabled").
    """
    result = {
        "face.detect": False,
        "body.pose": False,
