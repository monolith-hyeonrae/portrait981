"""Tests for momentbank.ingest bridge."""
...
from momentbank.paths import get_bank_path, _shard, list_member_ids
...
def patch_bank_base(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "momentbank.paths.get_home_dir",
        lambda: tmp_path,
    )
    # Also patch the import inside ingest.py
    monkeypatch.setattr(
        "momentbank.ingest.get_bank_path",
        lambda mid: tmp_path / "momentbank" / _shard(mid) / mid / "memory_bank.json",
    )
    return tmp_path
...
class TestIngestCollection:
    def test_basic_roundtrip(self, tmp_path: Path, patch_bank_base):
        ...
        bank_path = tmp_path / "momentbank" / shard / "test_user" / "memory_bank.json"
    ...
    def test_cumulative_update(self, tmp_path: Path, patch_bank_base):
        ...
        bank_path = tmp_path / "momentbank" / shard / "cumul_user" / "memory_bank.json"
        loaded = load_bank(bank_path)
...
class TestListMemberIds:
    def test_empty_base(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("momentbank.paths.get_bank_base_dir", lambda: tmp_path / "nonexistent")
        assert list_member_ids() == []

    def test_lists_members(self, tmp_path, monkeypatch):
        """Create several member banks and verify listing."""
        base = tmp_path / "momentbank"
        monkeypatch.setattr("momentbank.paths.get_bank_base_dir", lambda: base)
