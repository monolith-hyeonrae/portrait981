"""MemberMemory — per-member person-conditioned distribution store.

Entity store + Online stats + ANN index를 조합한 person memory.

세 가지 primitive:
  1. Entity store: member profile (JSON/pickle)
  2. Online stats: Welford (μ, Σ incremental update)
  3. ANN index: face embedding 검색 (numpy, 향후 FAISS)

Usage:
    memory = MemberMemory(member_id="test_3")
    memory.ingest(workflow_id="ride_1", frames=shoot_results, summary=signal_summary)
...
logger = logging.getLogger("personmemory.member")
...
class MemberProfile:
    ...
    signal_mean: np.ndarray          # 43D global mean
    signal_std: np.ndarray           # 43D global std
    coverage: dict[str, bool]        # {expr|pose: True/False}
    gaps: list[str]                  # 비어있는 버킷


class MemberMemory:
    """Per-member person memory — conditional distribution store.
    ...
    def __init__(self, member_id: str, store_dir: Optional[Path] = None):
        ...
        self.store_dir = store_dir or Path(f"data/personmemory/members/{member_id}")
        ...
        self._global_m2: Optional[np.ndarray] = None  # sum of squared deviations

        # Face embeddings (dominant identity)
    ...
    def ingest(
        ...
        Args:
            ...
            summary: SignalSummary (optional, 분포 일괄 업데이트용)
            timestamp: ISO timestamp

        Returns:
            dict with ingest stats (new_nodes, updated_nodes, n_frames)
        """
        ...
        for frame in frames:
            ...
            if conf > node.best_confidence:
                node.best_confidence = conf
        ...
        logger.info(
            "Ingest %s workflow=%s: %d frames, %d new nodes, %d updated",
            self.member_id, workflow_id, len(frames), new_nodes, updated_nodes,
        )
    ...
    def get_reference(
        ...
        Args:
            expression: 원하는 표정 (None이면 전체)
            pose: 원하는 포즈 (None이면 전체)
            top_k: 최대 반환 수

        Returns:
            이미지 경로 리스트 (confidence 순)
        """
        candidates = []
    ...
    def get_profile_reference(self, top_k: int = 1) -> list[str]:
        ...
        가장 많이 관찰된 expression×pose 노드의 best frame.
        """
        if not self.nodes:
            return []

        # n_observed × confidence로 정렬
        ranked = sorted(
            self.nodes,
            key=lambda n: n.n_observed * n.best_confidence,
            reverse=True,
        )
    ...
    def profile(self) -> MemberProfile:
        """Member 전체 프로필."""
        ...
        return MemberProfile(
            ...
            expression_dist=expr_dist,
            pose_dist=pose_dist,
            signal_mean=global_mean,
            signal_std=global_std,
            coverage=coverage,
            gaps=gaps,
        )
    ...
    def marginal_value(self, signal: np.ndarray) -> float:
        ...
        Mahalanobis-like distance (diagonal approximation).
        값이 클수록 기존 기억에 없는 새로운 표현.
        """
        if self._global_n < 2 or self._global_m2 is None:
            return 1.0  # 데이터 부족 → 모든 프레임이 가치 있음

        var = self._global_m2 / (self._global_n - 1)
        var = np.maximum(var, 1e-8)  # zero division 방지
    ...
    def save(self) -> Path:
        ...
        logger.info("Saved member memory: %s (%d nodes)", self.member_id, len(self.nodes))
        return path

    @classmethod
    def load(cls, member_id: str, store_dir: Optional[Path] = None) -> MemberMemory:
        """디스크에서 member 기억 로딩."""
        store = store_dir or Path(f"data/personmemory/members/{member_id}")
        path = store / "memory.json"

        mem = cls(member_id=member_id, store_dir=store)
        if not path.exists():
            return mem

        data = json.loads(path.read_text(encoding="utf-8"))
        mem.n_visits = data.get("n_visits", 0)
        mem.n_total_frames = data.get("n_total_frames", 0)
        mem._global_n = data.get("global_n", 0)

        if "global_mean" in data:
            mem._global_mean = np.array(data["global_mean"])
            mem._global_m2 = np.array(data["global_m2"])

        if "dominant_embedding" in data:
            mem._dominant_embedding = np.array(data["dominant_embedding"])

        for nd in data.get("nodes", []):
            mem.nodes.append(MemoryNode(
        ...
        logger.info("Loaded member memory: %s (%d nodes, %d visits)",
                    member_id, len(mem.nodes), mem.n_visits)
        return mem
    ...
    def _save_frame(self, image: np.ndarray, expr: str, pose: str,
        ...
        """프레임 이미지를 member 디렉토리에 저장."""
        import cv2
        images_dir = self.store_dir / "frames"
