...
from p981.core.bootstrap import CoreApp
from p981.core.application.observability.observer_progress import ObserverProgressSink
from p981.core.application.protocols import ObserverEvent, ObserverProtocol
from p981.core.application.stage.discover import DiscoverInput
from p981.core.application.stage.synthesize import SynthesizeInput
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(prog="p981.core.cli")
    parser.add_argument(
        "--mode",
        default="dev",
    ...
    parser.add_argument(
        ...
        action="append",
        help=(
            "Observer backend (noop/log/frames/pixeltable/opencv/rerun). "
...
def _build_progress_sink(
    args: argparse.Namespace, observer: ObserverProtocol, log_level: str
) -> ObserverProgressSink | None:
    """요청 시(또는 debug) 진행률 출력 핸들러를 만든다."""

    if args.progress or args.mode == "debug":
        return ObserverProgressSink(observer, level=log_level)
    return None
...
def main() -> None:
    ...
    # observer/executor setting
    app = CoreApp.from_wiring(args.mode, _parse_observers(args.observer))
    observer = app.observer
    progress = _build_progress_sink(args, observer, log_level)

    if args.mode == "prod":
        _emit_log(observer, "WARNING", "prod mode uses in-memory stores in the skeleton implementation.")
    ...
    if args.command == "discover":
        ...
        _emit_log(observer, "WARNING", "Unknown command", command=args.command)
        return
