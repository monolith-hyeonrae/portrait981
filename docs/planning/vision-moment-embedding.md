# Moment Embedding — momentbank 기반 고객 이해 임베딩
...
momentbank에 축적된 고객 이해 — 표정 패턴, 포즈 선호, 스타일 반응, 상태 변화 —
...
embeddings_042 = momentbank.get_face_embeddings("park_042")
...
```
입력:  이미지 + signal + momentbank profile
...
3. **momentbank = 학습 데이터 생성기** — 서비스가 돌수록 데이터가 쌓이고 모델이 진화
