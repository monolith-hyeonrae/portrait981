...
momentscan → visualbind → visualgrow의 3단계는
**단일 모델을 연구할 리소스가 없는 조직이 시스템을 단계적으로 도입하는 구조**다.

```
Day 1:  frozen model을 가져다 쓴다 (momentscan)
        → 즉시 동작, 수동 임계값으로 시작
...
momentscan만으로 동작 가능, visualbind는 선택적 개선, visualgrow는 미래 확장.
...
```
┌─────────────────────────────────────────────────────────┐
│ Layer 1: momentscan (Signal Production)                 │
│                                                          │
...
visualbind는 momentscan이 생산한 signal을 받아서 해석하지,
...
scripts/가 momentscan 파이프라인을 완전히 우회하고 있음을 발견.

### 문제 구조
```
올바른 흐름 (설계 의도):
  비디오 → momentscan DAG → 49D signal + face.gate
                                    ↓
...
실제 구현 (scripts/):
  ...
  momentscan의 DAG, 의존성 관리, face.quality 마스크 기반 측정,
  face.gate 다중 조건이 전부 무시됨.
```

### 구체적 누락 사항
| momentscan에 있는 것 | scripts에 없었던 것 |
|---------------------|-------------------|
...
### 방안 A: momentscan에 이미지 모드 추가

```python
# momentscan이 비디오와 이미지 모두 처리
results = momentscan.run(input="video.mp4", fps=2)     # 기존
results = momentscan.run(input="images/*.jpg")          # 신규
...
단점: momentscan이 비디오 프레임워크에 강하게 결합되어 있어 수정 범위 큼

### 방안 B: signal 추출 함수를 공유 모듈로 분리
...
단점: momentscan 내부 구조 노출

### 방안 C: vpx-runner 활용

```python
# vpx run이 이미 DAG 실행을 지원
# 이미지 입력 모드를 vpx-runner에 추가
signals = vpx.run(
    analyzers=["face.detect", "face.au", "face.expression", ...],
    input="image.jpg",
    output="signals",
)
```
...
장점: 기존 인프라 활용
단점: momentscan 플러그인 (face.quality, face.gate)은 vpx가 아닌 momentscan 영역

### 추천: 방안 B

momentscan 안에 `SignalExtractor` 클래스를 만들어서:
1. analyzer 로딩 + 의존성 관리를 재사용
2. face.quality, face.gate를 포함한 완전한 signal 추출
3. 비디오 모드와 이미지 모드 모두 지원
4. scripts/는 이걸 import해서 사용
