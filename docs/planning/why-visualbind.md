...
두 개의 독립적 view가 서로를 supervision하는 **co-training**과 이론적으로 연결됩니다. visualbind에서 각 analyzer는 독립된 view이며, agreement는 multi-view consistency를 측정합니다. 차이점: co-training은 2-view를 가정하지만, visualbind는 N-observer를 일반화합니다.

### Prototypical Networks (Snell et al., 2017)
기존 momentscan의 catalog_scoring은 본질적으로 **hand-crafted prototypical network**입니다 — category centroid에 대한 weighted distance 계산. visualbind는 이 prototype matching을 **agreement-supervised metric space**에서 data-driven으로 전환합니다.
...
## 뇌신경학적 기반 (전문가 리뷰 2026-03-12)

visualbind의 "합의 기반 결합"은 단순한 메타포가 아니라, 뇌신경학에서 검증된 원리와 구조적으로 대응합니다.

### Multisensory Integration (MSI)과 Inverse Effectiveness

뇌의 다감각 통합에서 가장 중요한 발견 중 하나:
**약한 단일 감각 신호일수록 다감각 통합의 이득(gain)이 크다.**

시각 신호만으로는 불확실하고, 청각 신호만으로도 불확실할 때 — 두 신호가 **일치**하면 반응 이득이 가장 큽니다. 강한 단일 감각 신호는 이미 충분한 정보를 가지고 있어 통합 이득이 작습니다.

visualbind에서 이 원리는 직접 적용됩니다: expression은 미약하고 AU도 미약하지만, 둘이 일치하면(both low) 그것은 "조용한 neutral 상태"라는 유의미한 정보입니다. 현재 `a * b` 함수는 이 원리를 위반합니다 — both-low를 무시합니다.

### FACS Temporal Dynamics

표정은 정적이 아닙니다. 자발적(spontaneous) 미소에서:
- **AU12** (lip corner puller)가 먼저 활성화
- **AU6** (cheek raiser, Duchenne marker)가 **67-170ms 후** 활성화

이 시간차는 30fps 비디오에서 **2-5 프레임**입니다. 프레임 단위로 AU6과 AU12를 비교하면 onset 구간에서 false conflict가 발생합니다. 이것이 temporal window가 필요한 이유입니다.

또한 표정의 생애주기:
- Onset (~500ms): 근육 활성화 시작
- Apex (가변): 최대 강도 유지
- Offset (~1000ms): 이완

각 phase에서 AU 비율이 다릅니다. 프레임 독립 비교는 이 dynamics를 놓칩니다.

### Duchenne Smile과 Conjunction 문제

진짜 미소(Duchenne smile)는 AU6 **AND** AU12의 **동시** 활성화로 정의됩니다. 현재 CrossCheck 구조는 1:1 관계만 표현 가능하여 이런 conjunction을 직접 표현할 수 없습니다.

### Superior Temporal Sulcus (STS)

뇌의 STS 영역은 얼굴의 다중 감각 정보(표정, 시선, 입 움직임)를 통합 처리합니다. 흥미로운 점은 STS가 **conflict detection**과 **quiet-state recognition**을 분리 처리한다는 것입니다. "모순"과 "조용함"은 다른 신경 경로를 탑니다. visualbind의 conflict/quiet/missing 3-way 분류 필요성의 신경학적 근거입니다.

### Valence-Arousal Dimensional Model

감정의 차원적 모델(Russell, 1980)은 모든 감정을 Valence(쾌-불쾌)와 Arousal(각성-이완) 2D 공간에 배치합니다. visualbind의 embedding space에서 이 V-A 축이 자연스럽게 나타나는지 분석하면, embedding의 해석 가능성과 이론적 정당성을 동시에 확보할 수 있습니다.
