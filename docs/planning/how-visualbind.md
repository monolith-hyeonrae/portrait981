...
```
                    ...
                    ▼                         ▼                         ▼
            ┌──────────────┐          ┌──────────────┐          ┌──────────────┐
            │  face.detect │          │  body.pose   │          │ hand.gesture │
            │  (SCRFD)     │          │  (YOLO)      │          │ (MediaPipe)  │
            └──────┬───────┘          └──────────────┘          └──────────────┘
                   │ face crop                                  독립 모듈 (N_eff↑)
      ┌────────────┼────────────┬────────────┐
      ▼            ▼            ▼            ▼
 ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐
 │face.au  │ │face.expr│ │head.pose│ │face.    │
 │(LibreFace)│ │(HSEmo) │ │(6DRep) │ │parse    │
 │ 12D     │ │  8D     │ │  3D    │ │         │
 └────┬────┘ └────┬────┘ └───┬────┘ └─────────┘
      │           │          │
      ▼           ▼          ▼         confidence 1D
 ┌──────────────────────────────────────────────┐
 │          25D Signal Vector                    │
 │  [AU×12 | Emotion×8 | yaw,pitch,roll | conf] │
 └──────────────────┬───────────────────────────┘
                    │
        ┌───────────┴───────────┐
        │                       │
...
```
        25D Signal
            │
     ┌──────▼──────┐
     │  LayerNorm   │  ← 이종 스케일 자동 정규화
     └──────┬──────┘
            │
     ┌──────▼──────────────────┐
     │    Shared Encoder        │
     │  25 → 64 → 32 → 16     │  ← ~3.5K params
     └──────┬──────────────────┘
            │
            ▼
     z (16D latent state)
     ...
     │                                          │
     ▼                                          ▼
  Teacher Heads (soft)                    Bucket Heads (hard)
  ┌────────────────────┐                ┌────────────────────┐
  │ Head₁ → AU 재구성   │                │ Head₁ → warm_smile? │
  │ Head₂ → Emotion 재구성│              │ Head₂ → cool_expression?  │
  │ Head₃ → Pose 재구성  │               │ Head₃ → lateral?    │
  │ Head₄ → Conf 재구성  │               │ ...                 │
  │ ...    (×14)        │                │ Head_k → 새 버킷?    │
  └─────────┬──────────┘                └─────────┬──────────┘
            │                                      │
            ▼                                      ▼
     Σ MSE / (2σ²ᵢ) + log σᵢ              Σ BCE / (2σ²ₖ) + log σₖ
            │                                      │
            └──────────────┬───────────────────────┘
                           │
                    ┌──────▼──────┐
                    │  + λ·VICReg  │
                    │  Total Loss  │  ← Kendall uncertainty weighting
                    └─────────────┘       σ² 가 자동으로 가중치 조절
```
...
Phase 1 (MVP)
═══════════════════════════════════════════════════
  Raw Image → [14 Teachers] → 25D → [Student MLP] → Bucket 판단
              ~~~~항상 실행~~~~       ~~~~~~~~~~~~~~~
              inference 필수          catalog_scoring 대체


Phase 2 (End-to-End)
═══════════════════════════════════════════════════
  학습:  Raw Image → [14 Teachers] → pseudo-label ─┐
         Raw Image → [Student CNN] ─────────────────┤→ Loss
                     (IResNet-18)                   │
  ───────────────────────────────────────────────────
  추론:  Raw Image → [Student CNN만] → Bucket 판단
                     ~~~~~~~~~~~~~~
                     Teachers 불필요!
...
Phase 3 (Foundation)
═══════════════════════════════════════════════════
  Raw Image → [Student] → z → Bucket / 다른 downstream task
              ~~~~~~~~~~
              도메인 foundation model
              visualpath 퇴역
```
...
visualbind는 이 발판이 만들어낸 pseudo-label을 소비하는 시스템이다.
Phase 간 모델 파라미터 전이는 없다 — 전이되는 것은 **pseudo-label 생성 파이프라인과 검증 인프라**이다.
Phase 1이 pseudo-label 품질을 검증하고, Phase 2가 검증된 pseudo-label로 새 모델을 학습한다.

```
Phase 1 (MVP):   visualpath → 25D signal → Student MLP → 판단 개선
...
```
Raw Image → [14 Teachers (항상 실행)] → 25D signal → [Student MLP] → 버킷 판단
```
...
학습 시: Raw Image → [14 Teachers] → pseudo-label (annotation 공장)
         Raw Image → [Student CNN] → 25D 예측 + 버킷 판단
...
Phase 2의 "Teacher를 annotation 공장으로 전환" 접근의 원리적 근거:
- **Florence-2** [Xiao 2024, CVPR]: 다양한 frozen 모델 출력으로 FLD-5B(54억 자동 annotation) 생성 → unified vision model 학습. "frozen 모델 출력으로 단일 모델을 학습할 수 있다"는 원리의 **대규모 검증 사례**. VisualBind와 비교 대상은 아니지만 (규모/인프라가 다른 차원), 동일 원리가 작동함을 보여준다.

직접적 선행연구:
- **Autodistill** [Roboflow 2023]: foundation model → pseudo-label → target model. Phase 2 구조와 가장 유사.
...
- Face Size: 프레임 대비 얼굴 크기 비율. portrait 품질 + face.detect 실패 모드 감지. 도메인 최적 크기를 Student가 학습.

CLIP/DINO 등 범용 임베딩 모델은 제외 — 얼굴 표정/포즈 변화에 대한 변별력 부족 확인.

### 입력 정규화: LayerNorm

25D signal의 스케일이 제각각이다 (AU: 0-5, Emotion: 0-1, Pose: -60°~60°).
...
- LayerNorm은 feature 차원(25D)에 대해 정규화 → 배치 크기 무관, train/eval 동일 동작
...
```
14개 Teacher 연속 출력 (25D)
         ↓
    [LayerNorm]
         ↓
[Shared Encoder] — MLP (25D → z, 2-3 hidden layers)
         ↓
...
[Teacher Head₂] → AU 예측            [Bucket Head₂] → "cool_expression" 적합?
...
| 구성요소 | 입력 → 출력 | 역할 |
|----------|------------|------|
| **LayerNorm** | 25D → 25D (정규화) | 스케일 통일, Teacher drift 적응, single-frame inference 안정 |
| **Shared Encoder** | 25D → z (16D) | model_bias를 보정한 버킷-불변 표현 학습 |
...
|----------|-----------|------|
| LayerNorm (25D) | ~50 | |
| Shared Encoder (25→64→32→16) | ~3.5K | |
...
#    예: 참조 이미지 분포의 5th percentile = 하한
thresholds = {
...
| `model` | 25D tensor → z, teacher preds, bucket preds | PyTorch |
...
이 조합의 장점:
- warm_smile vs cool_expression: **표정만 다름** → expression/AU 모듈의 bias 분리
- warm_smile vs lateral: **포즈가 다름** → pose 모듈의 bias 분리
- 25D signal 중 어떤 차원이 어떤 구분에 기여하는지 명확히 관찰 가능
...
```
Exp 6:  Raw image + CNN backbone vs Phase 1 best (25D MLP)
